# Name: Fuzail Fazal
# Date: 6-8-2026
#Changes Made: cleaned up the gui, changed difficulty names from easy,medium,hard to beginner, advanced, expert. And Added a level bar.
...
def settings():
    ...
    card = tk.Frame(win, bg="white", highlightbackground=CARD_BORDER,
                    highlightthickness=1)
    card.pack(fill="both", expand=True, padx=55, pady=38)
 
    user = current_player.username if current_player else "Not logged in"
    tk.Label(card, text="Account", font=SMALL_FONT, bg="white",
             fg=MUTED_TEXT).pack(anchor="w", padx=30, pady=(28, 3))
    tk.Label(card, text=user, font=HEADING_FONT, bg="white",
             fg=TEXT_COLOUR).pack(anchor="w", padx=30)
 
    tk.Frame(card, bg=CARD_BORDER, height=1).pack(fill="x", padx=30, pady=25)
    tk.Label(card, text="Quiz music", font=SUBHEADING_FONT,
             bg="white", fg=TEXT_COLOUR).pack(anchor="w", padx=30)
    tk.Label(card, text="Music plays only during quizzes. Drag to change its volume.",
             font=SMALL_FONT, bg="white", fg=MUTED_TEXT).pack(anchor="w", padx=30)
 
    vol = tk.Scale(card, from_=0, to=100, orient="horizontal", length=390,
                   bg="white", fg=TEXT_COLOUR, troughcolor=SOFT_PURPLE,
                   activebackground=MAIN_PURPLE, highlightthickness=0,
                   command=change_volume)
    vol.set(int(music_volume * 100))
    vol.pack(pady=18)
 
    status = "Quiz music is playing" if music_loaded else "Quiz music is currently stopped"
    tk.Label(card, text=status, font=SMALL_FONT, bg="white",
             fg=GREEN if music_loaded else MUTED_TEXT).pack()
...
def profile_screen():
    ...
    values = [("Level", str(level)), ("Total XP", str(total_xp)),
              ("Accuracy", str(get_accuracy(correct, answered)) + "%"),
              ("Answered", str(answered)), ("Correct", str(correct)),
              ("Incorrect", str(wrong)), ("Playtime", time_text(total_time)),
              ("Best mode", max(GAME_MODES, key=lambda m: stats[user][m]["xp"]))]
    for i, item in enumerate(values):
        stat_card(cards, item[0], item[1]).grid(
            row=i // 4, column=i % 4, padx=8, pady=8, sticky="nsew")
 
    detail = tk.Frame(win, bg="white", highlightbackground=CARD_BORDER,
                      highlightthickness=1)
    detail.pack(fill="both", expand=True, padx=42, pady=(0, 28))
    tk.Label(detail, text="XP by mode", font=SUBHEADING_FONT, bg="white",
             fg=TEXT_COLOUR).pack(anchor="w", padx=22, pady=14)
    for i, mode in enumerate(GAME_MODES):
        colour = SOFT_PURPLE if i % 2 == 0 else "white"
        row = tk.Frame(detail, bg=colour)
        row.pack(fill="x")
        tk.Label(row, text=mode, width=22, anchor="w", font=BODY_FONT,
                 bg=colour, fg=TEXT_COLOUR).pack(side="left", padx=20, pady=8)
        tk.Label(row, text=str(stats[user][mode]["xp"]) + " XP",
                 width=12, font=SUBHEADING_FONT, bg=colour,
                 fg=HEADER_PURPLE).pack(side="right", padx=20)
