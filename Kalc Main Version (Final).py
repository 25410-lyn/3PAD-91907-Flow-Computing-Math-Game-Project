# Name: Fuzail Fazal
# Date: 6-8-2026
#Changes Made: cleaned up the gui, changed difficulty names from easy,medium,hard to beginner, advanced, expert.
...
def page_header(parent, title, subtitle="", back_command=None):
    #a shared purple header used at the top of the larger screens
...
def stat_card(parent, title, value, colour=SOFT_PURPLE):
    # small card used by the profile and quiz screens to show the user's stats
...
def validate_fraction(text):

    pattern = r"^[+-]?\d+/[+-]?\d+$"

    if not re.fullmatch(pattern, text):
        return False, "Enter a fraction in the format x/y, for example 3/4."
    ...
    try:
        ...
        if denominator_value == 0:
            return False, "The denominator cannot be zero."

        Fraction(int(numerator), denominator_value)

    except (ValueError, ZeroDivisionError):
        return False, "Enter a valid fraction in the format x/y."

    return True, ""


def validate_calculus_term(text, integration=False):

    if integration:
        pattern = r"^[+-]?(?:\d+(?:\.\d+)?|\.\d+)x\^[+-]?(?:\d+(?:\.\d+)?|\.\d+)\+ ?c$"

        if not re.fullmatch(pattern, text):
            return False, "Enter the answer in the format Ax^b+c, for example 2x^3+c."

        # Remove the integration constant so that only Ax^b remains for further validation.
        term = text[:-2].rstrip()

        # If the answer ends with "+ c" this will remove the space as well.
        if term.endswith("+"):
            term = term[:-1]
        ...
        if not re.fullmatch(pattern, text):
            return False, "Enter the answer in the format Ax^b, for example 2x^3."

        term = text

    if integration:
        term_pattern = r"^([+-]?(?:\d+(?:\.\d+)?|\.\d+))x\^([+-]?(?:\d+(?:\.\d+)?|\.\d+))$"
    else:
        term_pattern = r"^([+-]?(?:\d+(?:\.\d+)?|\.\d+))x\^([+-]?(?:\d+(?:\.\d+)?|\.\d+))$"

    match = re.fullmatch(term_pattern, term)

    if match is None:
        return False, "Enter the answer in the required Ax^b format."

    coefficient = match.group(1)
    power = match.group(2)

    if not number_is_valid(coefficient):
        return False, "The coefficient A must be a number."

    if not number_is_valid(power):
        return False, "The power b must be a number."
...
def answer_is_valid(text, mode):
    ...
    if mode in [
        ...
        if not number_is_valid(cleaned):
            return False, "Enter a number only, for example 12, -4 or 3.5."

        return True, ""

    if mode == "Fractions":
        return validate_fraction(cleaned)

    if mode == "Differentiation":
        return validate_calculus_term(cleaned, False)

    if mode == "Integration":
        return validate_calculus_term(cleaned, True)

    return False, "Invalid question mode."
...
def answers_match(user_answer, correct_answer, mode):
    ...
    try:
        ...
        if mode in ["Addition","Subtraction","Multiplication","Division","Algebra"]:
            user_valid, _ = answer_is_valid(user, mode)
            correct_valid, _ = answer_is_valid(correct, mode)

            if not user_valid or not correct_valid:
                return False
        ...
        if mode == "Integration":
            ...
            # The only permitted difference is the optional space before the "+ c" in the answer. Normalize both answers to remove this space for comparison.
            user_normalised = user.replace("+ c", "+c")
            correct_normalised = correct.replace("+ c", "+c")

            return user_normalised == correct_normalised
...
def start_music():
    # starts music only when a quiz opens and safely handles missing resources
    global music_loaded
...
def stop_music():
    # stops quiz music safely when the quiz closes or finishes
    global music_loaded
 
    if MUSIC_OK == False or music_loaded == False:
        return
...
def close_game():
    ...
    root.destroy()
def settings():
    #This is the settings window which contains the music volume adjustor.
    win = tk.Toplevel(root)
    win.title("Kalc | Settings")
    win.geometry("620x500")
    win.resizable(False, False)
    win.configure(bg=PALE_PURPLE)
    page_header(win, "Settings", "Sound and account preferences", win.destroy)
 
    card = tk.Frame(win, bg="white", highlightbackground=CARD_BORDER,highlightthickness=5)
    card.pack(fill="both", expand=True, padx=55, pady=38)
 
    user = current_player.username if current_player else "Not logged in"
    tk.Label(card, text="Account", font=SMALL_FONT, bg="white",fg=MUTED_TEXT).pack(anchor="w", padx=30, pady=(28, 3))
    tk.Label(card, text=user, font=HEADING_FONT, bg="white",fg=TEXT_COLOUR).pack(anchor="w", padx=30)
 
    tk.Frame(card, bg=CARD_BORDER, height=1).pack(fill="x", padx=30, pady=25)
    tk.Label(card, text="Quiz music", font=SUBHEADING_FONT,bg="white", fg=TEXT_COLOUR).pack(anchor="w", padx=30)
    tk.Label(card, text="Music plays only during quizzes. Drag to change its volume.",font=SMALL_FONT, bg="white", fg=MUTED_TEXT).pack(anchor="w", padx=30)
 
    vol = tk.Scale(card, from_=0, to=100, orient="horizontal", length=390,bg="white", fg=TEXT_COLOUR, troughcolor=SOFT_PURPLE,activebackground=MAIN_PURPLE, highlightthickness=0,command=change_volume)
    vol.set(int(music_volume * 100))
    vol.pack(pady=18)
 
    status = "Quiz music is playing" if music_loaded else "Quiz music is currently stopped"
    tk.Label(card, text=status, font=SMALL_FONT, bg="white",fg=GREEN if music_loaded else MUTED_TEXT).pack()
...
def leaderboard():
    ...
    board = tk.Frame(win, bg="white", highlightbackground=CARD_BORDER,highlightthickness=5)
    ...
    def show_board(mode):
        ...
        for place, row in enumerate(rows[:10], 1):
            ...
            for value, width in zip(values, widths):
                tk.Label(line, text=value, width=width, font=BODY_FONT,bg=colour, fg=TEXT_COLOUR).pack(side="left", pady=13)
 
    for i, mode in enumerate(["Overall"] + GAME_MODES):
        tk.Button(tabs, text=mode, font=SMALL_FONT, relief="flat", bd=0,cursor="hand2", bg="white", fg=HEADER_PURPLE,activebackground=SOFT_PURPLE,command=lambda m=mode: show_board(m)).grid(row=i // 5, column=i % 5, padx=4, pady=4, ipadx=10, ipady=7)
    show_board("Overall")
 
 
def profile_screen():
    # dashboard layout inspired by modern game profile screens
    win = tk.Toplevel(root)
    win.title("Kalc | Profile")
    fit_window(win, 920, 760)
    win.resizable(False, False)
    win.configure(bg=PALE_PURPLE)
 
    user = current_player.username if current_player else "Unknown"
    page_header(win, "Profile", user + "'s learning statistics", win.destroy)
 
    stats = read_leads()
    make_user_stats(stats, user)
    total_xp = sum(stats[user][m]["xp"] for m in GAME_MODES)
    answered = sum(stats[user][m]["answered"] for m in GAME_MODES)
    correct = sum(stats[user][m]["correct"] for m in GAME_MODES)
    total_time = sum(stats[user][m]["time"] for m in GAME_MODES)
    wrong = answered - correct
    level = get_level(total_xp)
    level_progress = total_xp % XP_PER_LEVEL
 
    cards = tk.Frame(win, bg=PALE_PURPLE)
    cards.pack(fill="x", padx=34, pady=22)
    values = [("Level", str(level)), ("Total XP", str(total_xp)),("Accuracy", str(get_accuracy(correct, answered)) + "%"),("Answered", str(answered)), ("Correct", str(correct)),("Incorrect", str(wrong)), ("Playtime", time_text(total_time)),("Best mode", max(GAME_MODES, key=lambda m: stats[user][m]["xp"]))]
    for i, item in enumerate(values):
        stat_card(cards, item[0], item[1]).grid(row=i // 4, column=i % 4, padx=8, pady=8, sticky="nsew")
 
    level_card = tk.Frame(win, bg="white", highlightbackground=CARD_BORDER,highlightthickness=5)
    level_card.pack(fill="x", padx=42, pady=(0, 18))
    tk.Label(level_card, text="Level progress", font=SUBHEADING_FONT,bg="white", fg=TEXT_COLOUR).pack(anchor="w", padx=22, pady=(15, 4))
    canvas = tk.Canvas(level_card, width=LEVEL_BAR_WIDTH, height=22, bg="white",highlightthickness=0)
    canvas.pack(padx=20, pady=(0, 16))
    canvas.create_rectangle(0, 2, LEVEL_BAR_WIDTH, 20, fill=DEEP_PURPLE, outline="")
    canvas.create_rectangle(0, 2, (LEVEL_BAR_WIDTH / XP_PER_LEVEL) * level_progress, 20,fill=ORANGE, outline="")
    canvas.create_text(LEVEL_BAR_WIDTH / 2, 11, text="{}/{} XP".format(
        level_progress, XP_PER_LEVEL), fill="white", font=SMALL_FONT)
 
    detail = tk.Frame(win, bg="white", highlightbackground=CARD_BORDER,highlightthickness=5)
    detail.pack(fill="both", expand=True, padx=42, pady=(0, 28))

    tk.Label(detail, text="XP by mode", font=SUBHEADING_FONT, bg="white",fg=TEXT_COLOUR).pack(anchor="w", padx=22, pady=14)

    scroll_frame = tk.Frame(detail, bg="white")
    scroll_frame.pack(fill="both", expand=True)

    canvas = tk.Canvas(scroll_frame, bg="white", highlightthickness=0)

    scrollbar = tk.Scrollbar(scroll_frame, orient="vertical", command=canvas.yview,bg=PALE_PURPLE, troughcolor=PALE_PURPLE,activebackground=SOFT_PURPLE, highlightthickness=0,bd=0, width=10)
    ...
    for i, mode in enumerate(GAME_MODES):
        colour = SOFT_PURPLE if i % 2 == 0 else "white"

        row = tk.Frame(rows_frame, bg=colour)
        row.pack(fill="x")

        tk.Label(row,text=mode,width=22,anchor="w",font=BODY_FONT,bg=colour,fg=TEXT_COLOUR).pack(side="left", padx=20, pady=8)

        tk.Label(row,text=str(stats[user][mode]["xp"]) + " XP",width=12,font=SUBHEADING_FONT,bg=colour,fg=HEADER_PURPLE).pack(side="right", padx=20, pady=8)
...
def start_quiz(mode, diff):
    ...
    questions = [make_question(mode, diff) for i in range(QUESTIONS_PER_QUIZ)]
    ...
    modern_button(header, "Quit", close_quiz, ORANGE, 9, 1).pack(side="right", padx=30, pady=28)
 
    stats_bar = tk.Frame(win, bg=PALE_PURPLE)
    stats_bar.pack(fill="x", padx=55, pady=18)
    question_stat = stat_card(stats_bar, "QUESTION", "1 / {}".format(QUESTIONS_PER_QUIZ))
    ...
    progress = tk.Canvas(win, width=PROGRESS_BAR_WIDTH, height=18, bg=PALE_PURPLE,highlightthickness=0)
    progress.pack()
 
    feedback = tk.Label(win, text="Enter your answer and press Submit",font=SUBHEADING_FONT, bg=SOFT_PURPLE,fg=HEADER_PURPLE, height=2)
    ...
    question_label = tk.Label(question_card, text="", font=("Segoe UI", 25, "bold"),bg=DEEP_PURPLE, fg="white", wraplength=800)
    question_label.pack(expand=True)
 
    answer_row = tk.Frame(win, bg=PALE_PURPLE)
    answer_row.pack(fill="x", padx=62, pady=20)
    tk.Label(answer_row, text="Your answer", font=SUBHEADING_FONT,bg=PALE_PURPLE, fg=TEXT_COLOUR).pack(anchor="w")
    answer_entry = tk.Entry(answer_row, font=("Segoe UI", 18), relief="flat",bg=ENTRY_COLOUR, fg=TEXT_COLOUR,insertbackground=HEADER_PURPLE)
    answer_entry.pack(fill="x", ipady=12, pady=(7, 4))
    hint = tk.Label(answer_row, text="", font=SMALL_FONT,bg=PALE_PURPLE, fg=MUTED_TEXT)
    hint.pack(anchor="w")
    ...
    def tick():
        ...
        if state["time_left"] <= 0:
            ...
            state["time_left"] -= 1
            state["timer_id"] = win.after(TIMER_INTERVAL, tick)
 
    def end_quiz():
        stop_timer()
        seconds = int(time.time() - state["start_time"])
        quiz_result = QuizResult(mode, state["xp"], state["answered"],state["correct"], seconds)
        save_quiz_result(quiz_result)
        stop_music()
        for widget in win.winfo_children():
            if widget != header:
                widget.destroy()
        result = tk.Frame(win, bg="white", highlightbackground=CARD_BORDER,highlightthickness=5)
        result.pack(expand=True, fill="both", padx=150, pady=70)
        tk.Label(result, text="Quiz complete!", font=TITLE_FONT, bg="white",fg=HEADER_PURPLE).pack(pady=(55, 8))
        tk.Label(result, text="{}/{} correct".format(state["correct"], QUESTIONS_PER_QUIZ),font=("Segoe UI", 30, "bold"), bg="white",fg=TEXT_COLOUR).pack(pady=10)
        tk.Label(result, text="You earned {} XP in {}".format(
            state["xp"], time_text(seconds)), font=BODY_FONT,
            bg="white", fg=MUTED_TEXT).pack(pady=8)
        modern_button(result, "Back to modes",lambda: [stop_music(), win.destroy(), mode_screen()],MAIN_PURPLE, 20, 2).pack(pady=(30, 8))
        modern_button(result, "View profile", profile_screen,BLUE, 20, 2).pack(pady=8)
 
    def next_question():
        state["qnum"] += 1
        if state["qnum"] == QUESTIONS_PER_QUIZ:
            end_quiz()
        else:
            show_question()
    ...
    def mark_answer(value):
        ...
        if value is not None and answers_match(value, q["answer"], mode):
            state["correct"] += 1
            state["xp"] += DIFF_XP[diff]
            feedback.config(text="Correct!  +{} XP".format(DIFF_XP[diff]),bg=SUCCESS_BG, fg="#167A3B")
            question_card.config(bg="#167A3B")
            question_label.config(bg="#167A3B")
        elif value is None:
            feedback.config(text="Time up the answer was " + q["answer"],bg=ERROR_BG, fg="#A51E39")
        else:
            feedback.config(text="Not quite the answer was " + q["answer"],bg=ERROR_BG, fg="#A51E39")
            question_card.config(bg="#8E1538")
            question_label.config(bg="#8E1538")
        win.after(FEEDBACK_DELAY, next_question)
    ...
    def show_question():
        ...
        question_stat.winfo_children()[1].config(
            text="{} / {}".format(state["qnum"] + 1, QUESTIONS_PER_QUIZ))
        correct_stat.winfo_children()[1].config(text=str(state["correct"]))
        xp_stat.winfo_children()[1].config(text=str(state["xp"]))
        progress.delete("all")
        progress.create_rectangle(0, 2, PROGRESS_BAR_WIDTH, 16, fill=DEEP_PURPLE, outline="")
        progress.create_rectangle(0, 2, (PROGRESS_BAR_WIDTH / QUESTIONS_PER_QUIZ) * state["qnum"], 16,fill=ORANGE, outline="")
        tick()
...
def mode_screen():
    ...
    for i, mode in enumerate(GAME_MODES):
        card = tk.Frame(grid, bg="white", highlightbackground=CARD_BORDER,highlightthickness=5, width=205, height=205)
...
def menu_screen():
    ...
    modern_button(header, user, profile_screen, MAIN_PURPLE, 14, 1).pack(side="right", padx=30, pady=28)
    ...
    tk.Label(hero, text="Choose from eight modes and slowly build up your skills.",font=BODY_FONT, bg=DEEP_PURPLE, fg="#DCD1F4").pack(anchor="w", padx=38)
    ...
    options = [("Leaderboard", "Compare XP rankings", BLUE, leaderboard),("Profile", "View your statistics", MAIN_PURPLE, profile_screen),("Settings", "Adjust quiz-only music", ORANGE, settings),("Quit", "Close Kalc safely", RED, close_game)]
    for i, item in enumerate(options):
        title, desc, colour, command = item
        card = tk.Frame(actions, bg="white", highlightbackground=CARD_BORDER,highlightthickness=5, width=215, height=210)
        ...
        modern_button(card, "Select", command, colour, 13, 1).pack(pady=24)
...
def main():
    ...
    load_logo()
    main_frame = tk.Frame(root, bg=CARD_COLOUR, relief="flat", width=620,height=610, highlightbackground=CARD_BORDER,highlightthickness=5)
    main_frame.place(relx=0.5, rely=0.5, anchor="center")
    main_frame.pack_propagate(False)
 
    tk.Label(main_frame, text="KALC", font=("Segoe UI", 42, "bold"),bg=CARD_COLOUR, fg=HEADER_PURPLE).pack(pady=(45, 2))
 
    form = tk.Frame(main_frame, bg=CARD_COLOUR)
    form.pack(pady=10)
    tk.Label(form, text="Username", font=SUBHEADING_FONT,bg=CARD_COLOUR, fg=TEXT_COLOUR).grid(row=0, column=0, sticky="w", pady=(4, 6))
    username_entry = tk.Entry(form, width=35, font=("Segoe UI", 13),relief="flat", bg=ENTRY_COLOUR, fg=TEXT_COLOUR)
    username_entry.grid(row=1, column=0, ipady=10, pady=(0, 18))
 
    tk.Label(form, text="Password", font=SUBHEADING_FONT,bg=CARD_COLOUR, fg=TEXT_COLOUR).grid(row=2, column=0, sticky="w", pady=(4, 6))
    password_entry = tk.Entry(form, width=35, font=("Segoe UI", 13), show="*",relief="flat", bg=ENTRY_COLOUR, fg=TEXT_COLOUR)
    ...
    modern_button(bottom, "Create account", acc_screen, MAIN_PURPLE, 18, 1).grid(row=0, column=0, padx=7)
    modern_button(bottom, "Forgot password", forgot_screen, BLUE, 18, 1).grid(row=0, column=1, padx=7)
